...
class SalesReadDetail extends React.Component {
    constructor(props) {
        super(props)
        this.state = { invoice_no: window.location.pathname.split('/')[3], soldItems: [], invoiceData: {}, redirect: false, redirectDashboard: false }
        this.soldItemsTable = this.soldItemsTable.bind(this)
    }

    componentDidMount() {
        let token = sessionStorage.getItem('token')
        axios.get(`https://backend-pos-tap.herokuapp.com/admin/sales/sales-details/${this.state.invoice_no}`, { headers: { 'Authorization': `Bearer ${token}` } })
            .then(res => {
                this.setState({
                    ...
                    <div className='row'>
                        ...
                        <div className='container col-7' style={{ display: 'inline-block', margin: '0', padding: '0' }}>
                            ...
                            <div className='row' style={{ margin: '0 0 2vh 0' }}>
                                ...
                                <div className='col-5'>
                                    <input type='text' name='paidStatus' value={this.state.invoiceData.paid_status} style={inputStyle} disabled={true} />
                                </div>
                            </div>
                        </div>
...
export default SalesReadDetail