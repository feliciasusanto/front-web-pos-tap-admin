...
class ProductAddNew extends React.Component {
    constructor(props) {
        super(props)
        this.state = { itemCode: '', itemName: '', itemBrand: '', itemSupplier: '', itemDescription: '', redirect: false, redirectDashboard: false }
    ...
    componentDidMount() {
        ...
        axios.get('https://backend-pos-tap.herokuapp.com/admin/check-role-access-valid/superuser-warehouse', { headers: { 'Authorization': `Bearer ${token}` } })
            .then(res => { })
            .catch((err) => {
    ...
    handleClickSubmit = (event) => {
        ...
        if (this.state.itemCode.trim().length < 3) {
            ...
            let token = sessionStorage.getItem('token')
            axios.post('https://backend-pos-tap.herokuapp.com/admin/products/add-new-product', { itemCode: this.state.itemCode.toUpperCase(), itemName: this.state.itemName, itemBrand: this.state.itemBrand, itemSupplier: this.state.itemSupplier, itemDescription: this.state.itemDescription }, { headers: { 'Authorization': `Bearer ${token}` } })
            .then((res) => {
                alert(`Barang dengan kode ${this.state.itemCode.toUpperCase()} berhasil ditambahkan.`)
                this.setState({
                    redirect: true
                })
            })
